...
function moveAvailToSelect(iMoveSet, oLstAvail, oLstSelect, oHiddenSelected, oHiddenSelectedNames, oHiddenAvailable){
    try {
        ...
        var postalCode;
        if ($('[id*=rdStatePostal_0]').length > 0) {
            postalCode = document.getElementById($('[id*=rdStatePostal_0]')[0].id);
        }
		...
		if (postalCode=='undefined' || postalCode == null)
		{
			...
			else
			{
			    ...
			    if (document.getElementById($('[id*=lblAvailFullDisplay]')[0].id) != null)
			        document.getElementById($('[id*=lblAvailFullDisplay]')[0].id).innerText = '';
			}
		...
		else
		{
		    ...
		    if (document.getElementById($('[id*=rdStatePostal_0]')[0].id).checked == true)
			{
				copyItems(oLstAvail,oLstSelect,iMoveSet);
				if (iMoveSet==1){
					removeSelectedItems(oLstAvail);
